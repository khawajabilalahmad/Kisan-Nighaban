...
from app.schemas.analysis import FarmAnalysisOutput, FarmAnalysisResponse
...
async def analyze_farm(farm_id: str, db: AsyncSession = Depends(get_db)):
    ...
    # 5. Generate Farm Analysis (AI with fallback)
    try:
...
@router.get("/{farm_id}/latest", response_model=FarmAnalysisResponse)
async def get_latest_analysis(farm_id: str, db: AsyncSession = Depends(get_db)):
    """
    Get the latest farm analysis for a specific farm.
    """
    # 1. Fetch Farm
    result = await db.execute(select(Farm).where(Farm.id == farm_id))
    farm = result.scalar_one_or_none()
    ...
    if not latest_assessment:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No farm analyses found for this farm")
        
    return latest_assessment

@router.get("/{farm_id}/history", response_model=List[FarmAnalysisResponse])
async def get_analysis_history(farm_id: str, db: AsyncSession = Depends(get_db)):
    """
    Get the farm analysis history for a specific farm over the last 7 days.
    """
    # 1. Fetch Farm
    result = await db.execute(select(Farm).where(Farm.id == farm_id))
    farm = result.scalar_one_or_none()
    ...
    result = await db.execute(
        select(FarmAnalysis)
        .where(FarmAnalysis.farm_id == farm_id)
        .where(FarmAnalysis.assessed_at >= seven_days_ago)
        .order_by(FarmAnalysis.assessed_at.desc())
    )
