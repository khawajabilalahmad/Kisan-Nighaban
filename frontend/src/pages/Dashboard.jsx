    ...
    <div className="flex-1 flex flex-col overflow-y-auto p-6 space-y-8 pb-32">
      ...
      <div className="w-full flex flex-col justify-between mb-8 mt-6 relative z-10">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-4xl font-black text-slate-800 dark:text-white drop-shadow-md">Salam, {user?.full_name?.split(' ')[0] || 'Kisan'}</h2>
          </div>
        </div>
        ...
        <div className="flex items-end justify-between mt-6">
          <div className="bg-white/60 dark:bg-black/40 backdrop-blur-md px-5 py-4 rounded-3xl border border-white/50 dark:border-white/10 text-slate-800 dark:text-white shadow-lg">
            <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Total Farms</p>
      ...
      {farmsList.length > 0 && (
        <div className="mb-4">
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg tracking-tight">Your Farms</h3>
            <button onClick={() => navigate('/farms')} className="text-sm font-bold text-primary hover:underline">View All</button>
          </div>
